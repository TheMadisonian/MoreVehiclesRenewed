...
namespace MoreVehicles.Patches
{
    ...
    internal static class VehicleManagerPatch
    {
        ...
        private sealed class VehicleManager_SimulationStepImpl : PatchBase
        {
            ...
            private static IEnumerable<CodeInstruction> Transform(IEnumerable<CodeInstruction> instructions)
            {
                // Expand chunk size from 1024 to 4096 so the 16-frame cycle (mask 0xF) covers
                // all 65536 vehicle IDs (16 × 4096 = 65536).  This keeps the frame offset
                // aligned with Vehicle.GetTargetFrame which divides by ModdedMaxVehicleCount,
                // ensuring simulation and rendering stay in sync.
                // Also expand the ExtraSimulationStep loop from 16384 to 65536.
                var step1 = CodeProcessor.ReplaceOperands(instructions, 1024, 4096);
                return CodeProcessor.ReplaceOperands(step1, VanillaMaxVehicleCount, ModdedMaxVehicleCount);
            }
        }
    }
}
